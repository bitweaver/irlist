	<div class="navbar">
		{form class="find" legend="Find in Incident Report entries"}
			...
			{/foreach}
					...
					{forminput}
						{html_options values="$project_array" output="$project_array" name=project selected=`$ihash.project` id=project}
					{/forminput}
					{formlabel label="Version" for="version"}
					{forminput}
						<input type="text" size="12" maxlength="10" name="version" id="version" value="{$ihash.version}" />
					{/forminput}
					{formlabel label="Status" for="status"}
					{forminput}
						{html_options values="$status_array" output="$status_array_p" name=status selected=`$ihash.status` id=status}
					{/forminput}
					{formlabel label="Priority" for="priority"}
					{forminput}
						{html_options values="$priority_array" output="$priority_array_p" name=priority selected=`$ihash.priority` id=priority}
					{/forminput}
